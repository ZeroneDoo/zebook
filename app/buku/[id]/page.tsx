...
export default function BookDetailPage() {
	...
	// Simulasi dompet user (Ganti sesuai data dinamis dari user profile/DB Anda nantinya)
	...
	// 			message: "Silakan masuk ke akun Anda untuk menyimpan buku ke rak.",
	// 		});
	// 		return;
	// 	}
	...
	// 	toast({
	// 		type: "info",
	// 		title: isBookmarked ? "Dihapus dari Rak" : "Disimpan ke Rak",
	// 		message: isBookmarked
	// 			? "Buku telah dihapus dari daftar koleksi Anda."
	// 			: "Buku berhasil ditambahkan ke rak bacaan Anda.",
	// 		duration: 2000
	// 	});
	// };
	...
	const handleConfirmBorrow = async () => {
		...
		try {
			const response = await fetch("/api/peminjaman-buku/pinjam", {
				...
				body: JSON.stringify({
					id_pengguna: session.user.id, // Pastikan ID pengguna tersedia di session
					id_buku: book.id_buku,
					metode: paymentMethod,
					tgl_pinjam: new Date().toISOString(), // Opsional, bisa dikosongkan karena backend otomatis nge-set hari ini
				}),
			});
	...
	// Cek kecukupan saldo berdasarkan metode aktif di layar utama modal
					...
					<div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
						...
						<div className="lg:col-span-1 flex flex-col items-center">
							<div className="relative w-64 h-96 rounded-2xl bg-white border border-[#D4C4AE]/60 shadow-[0_16px_40px_rgba(58,40,24,.12)] overflow-hidden group">
								...
								<div className={`absolute top-4 right-4 z-30 font-bold uppercase tracking-widest text-[11px] px-2.5 py-1 border-2 rounded-md mix-blend-multiply opacity-90 -rotate-12 select-none pointer-events-none transition-all duration-500 scale-110 shadow-xs ${!isStokTersedia
									? "border-red-700 text-red-800 bg-red-50/70"
									: isBorrowed
										? "border-amber-700 text-amber-800 bg-amber-50/70 font-extrabold animate-in fade-in zoom-in-120 duration-300"
										: "border-emerald-700 text-emerald-800 bg-emerald-50/70"
									}`}>
									{!isStokTersedia ? "Habis" : isBorrowed ? "Dipinjam" : "Tersedia"}
								</div>
							...
							{/* <button
								...
								className={`mt-6 w-64 flex items-center justify-center gap-2 py-3 rounded-xl text-xs font-bold tracking-wide transition-all border cursor-pointer ${isBookmarked
									? "bg-[#FAF7F2] border-[#56402A] text-[#56402A]"
									: "bg-white border-[#D4C4AE] text-[#9A7E5A] hover:text-[#56402A] hover:border-[#56402A] shadow-sm"
									}`}
							>
								{isBookmarked ? <IconBookmarkFilled size={16} className="text-[#56402A]" /> : <IconBookmark size={16} />}
								{isBookmarked ? "Tersimpan di Rak" : "Tambah ke Rak Bacaan"}
							</button> */}
						</div>
						...
						<div className="lg:col-span-2 space-y-6">
							...
							<div className="bg-white rounded-[20px] border border-[#D4C4AE]/40 p-6 shadow-[0_4px_24px_rgba(58,40,24,.02)]">
								...
								<div className="flex items-center gap-4 pt-2 border-t border-[#E8DDD0]/50 mt-4 text-xs font-medium text-[#9A7E5A]">
									{/* <div className="flex items-center gap-1">
										...
										<span className="text-[#3A2818] font-bold">4.8</span>
										<span>(Terpuji)</span>
									</div>
							...
							<div className="bg-[#56402A] rounded-[20px] p-6 text-[#F3ECE0] flex flex-col sm:flex-row items-center justify-between gap-4 shadow-[0_12px_32px_rgba(86,64,42,.15)]">
								...
								{isBorrowed ? (
									...
									<button
										...
										className="w-full sm:w-auto flex items-center justify-center gap-2 bg-amber-500 hover:bg-amber-400 disabled:bg-stone-500 disabled:text-stone-300 disabled:opacity-50 text-[#3A2818] px-8 py-3.5 rounded-xl text-sm font-bold shadow-md transition-all cursor-pointer"
									>
										<IconClock size={18} />
							...
							<div className="bg-white rounded-[20px] border border-[#D4C4AE]/40 p-6 sm:p-8 space-y-4 shadow-[0_4px_24px_rgba(58,40,24,.02)]">
								...
								<p className="text-sm text-[#3A2818] leading-relaxed font-normal whitespace-pre-line">
									{book.deskripsi || "Tidak ada sinopsis resmi untuk kompilasi buku ini."}
								</p>
							</div>
							...
							<div className="bg-white rounded-[20px] border border-[#D4C4AE]/40 p-6 shadow-[0_4px_24px_rgba(58,40,24,.02)] space-y-4">
								...
								<div className="grid grid-cols-2 gap-y-4 gap-x-6 text-xs border-t border-[#E8DDD0]/50 pt-4">
									...
									<div className="space-y-1">
										<span className="text-[#9A7E5A] block font-medium">Bahasa</span>
										<span className="text-[#3A2818] font-semibold flex items-center gap-1.5">
											<IconLanguage size={14} className="text-[#9A7E5A]" /> Indonesia
										</span>
									</div>
									<div className="space-y-1">
										<span className="text-[#9A7E5A] block font-medium">Kode Registrasi</span>
										<span className="text-[#3A2818] font-mono font-semibold flex items-center gap-1.5 text-amber-800">
											#{book.id_buku}
										</span>
									</div>
								</div>
					...
					<div className="fixed inset-0 z-9999 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4">
						<div className="bg-white max-w-sm w-full rounded-[24px] border border-[#D4C4AE]/50 p-6 space-y-5 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
							...
							<div className="flex gap-3">
								...
								{/* Tombol Konfirmasi Pinjam sekarang menggunakan properti disabled={!isBalanceEnough} 
                  beserta styling state Tailwind disabled:
                */}
								<button
